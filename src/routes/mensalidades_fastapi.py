...
from src.models.financeiro import Financeiro # Importe o Financeiro
...
def read_mensalidades(
    ...
    # Usamos joinedload para carregar os dados do aluno e do plano junto com a mensalidade
...
def processar_pagamento_mensalidade(mensalidade_id: int, db: Session = Depends(get_db)):
    """
    Processa o pagamento de uma mensalidade e cria uma transação no financeiro.
    """
    db_mensalidade = db.query(Mensalidade).filter(Mensalidade.id == mensalidade_id).first()
    ...
    nova_transacao = Financeiro(
        ...
        descricao=f"Pagamento da mensalidade ID {db_mensalidade.id} do aluno {db_mensalidade.aluno.nome}",
