...
from src.schemas import usuario as schemas_usuario
...
async def login_staff_google(request: Request):
    redirect_uri = "http://localhost:8000/api/v1/auth/staff/callback/google" # URL Explícita
...
async def auth_staff_google_callback(request: Request, db: Session = Depends(database.get_db)):
    ...
    if not user:
        user = models.usuario.Usuario(email=email, nome=user_info_google.get('name'), role='pendente')
        db.add(user)
        db.commit()
        db.refresh(user)
...
async def login_aluno_google(request: Request):
    redirect_uri = "http://localhost:8000/api/v1/auth/aluno/callback/google" # URL Explícita
    return await auth.oauth.google.authorize_redirect(request, redirect_uri)
...
async def auth_aluno_google_callback(request: Request, db: Session = Depends(database.get_db)):
    ...
    if not aluno:
        aluno = models.aluno.Aluno(email=email, nome=user_info_google.get('name', 'Aluno Google'))
        db.add(aluno)
        db.commit()
        db.refresh(aluno)
