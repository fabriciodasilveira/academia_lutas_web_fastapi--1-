...
def financeiro_dashboard():
    ...
    return render_template("financeiro/dashboard.html", stats=stats, transacoes=transacoes, categorias=categorias)
...
# # ... (O restante do seu app.py não precisa ser alterado, apenas a rota abaixo) ...
...
#     # Faz a requisição para a API de balanço
#     balanco_response = api_request(f"/financeiro/balanco?data_inicio={primeiro_dia_mes.strftime('%Y-%m-%d')}&data_fim={hoje.strftime('%Y-%m-%d')}")
...
#     # Buscamos as transações recentes
#     transacoes_response = api_request("/financeiro/transacoes?limit=5")
#     transacoes = []
#     if transacoes_response and transacoes_response.status_code == 200:
#         transacoes_data = transacoes_response.json()
...
#     endpoint = "/financeiro/transacoes"
#     query_params = []
#     if tipo_filtro: query_params.append(f"tipo={tipo_filtro}")
#     if categoria_filtro: query_params.append(f"categoria={categoria_filtro}")
#     if busca_filtro: query_params.append(f"busca={busca_filtro}")
#     if data_inicio_filtro: query_params.append(f"data_inicio={data_inicio_filtro}")
#     if data_fim_filtro: query_params.append(f"data_fim={data_fim_filtro}")
...
#     # Buscar dados para o resumo e passar para o template
#     balanco_response = api_request("/financeiro/balanco")
#     stats = {}
#     if balanco_response and balanco_response.status_code == 200:
#         stats = balanco_response.json()
...
# # ... (O restante do seu app.py, como rotas de produtos, planos, etc.) ...
